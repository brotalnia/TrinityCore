...
void WorldSession::HandleAreaTriggerOpcode(WorldPackets::AreaTrigger::AreaTrigger& packet)
{
    ...
    if (!teleported)
    {
        ...
        if (entranceLocation)
            player->TeleportTo(entranceLocation->Location.GetMapId(), entranceLocation->Location.GetPositionX(), entranceLocation->Location.GetPositionY(),
                               entranceLocation->Location.GetPositionZ(), entranceLocation->Location.GetOrientation() * M_PI / 180, TELE_TO_NOT_LEAVE_TRANSPORT);
        else
